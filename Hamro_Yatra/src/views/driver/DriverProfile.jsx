...
function DriverProfile() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8080/driver/profiledata?email=${email}`)
            .then(response => {
                ...
                <div className="default-ride-data">
                    {defaultRide ? (
                        <div>
                            ...
                            <p>Days of Week: {defaultRide.daysOfWeek.map((day, index) => (
                                <React.Fragment key={index}>
                                    {day}{index < defaultRide.daysOfWeek.length - 1 && ', '}
                                </React.Fragment>
                            ))}</p>
                        </div>
                    ) : (
